...
const platforms = [
  ...
  },
  {
    name: 'X',
    color: '#000000',
    getUrl: (url: string) => `https://x.com/intent/tweet?url=${encodeURIComponent(url)}`,
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-6 w-6">
...
export function ShareModal({ postUrl, open, onOpenChange }: ShareModalProps) {
  return (
      ...
      <DialogContent className="sm:max-w-xs">
        ...
        <div className="grid grid-cols-4 gap-3 py-2">
          {platforms.map((platform) => (
            <button
              key={platform.name}
              onClick={() => window.open(platform.getUrl(postUrl), '_blank', 'noopener,noreferrer')}
