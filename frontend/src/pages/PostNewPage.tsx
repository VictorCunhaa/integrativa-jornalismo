...
import { useCreatePost, usePublishPost, useTaxonomies } from '@/hooks/usePosts'
...
export function PostNewPage() {
  ...
  const [format, setFormat] = useState(searchParams.get('format') || 'text')
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => save(false)} disabled={createPost.isPending}>
            Salvar rascunho
          </Button>
          <Button size="sm" onClick={() => save(true)} disabled={createPost.isPending}>
            Publicar
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
        ...
        <div className="space-y-4">
          ...
          />
          <PostEditor
            value={{ html: content.html as string, json: content.json }}
            onChange={setContent}
          />
        </div>
              ...
              <RadioGroup value={format} onValueChange={setFormat} className="space-y-1">
                {[['text', 'Texto'], ['photo', 'Foto'], ['audio', 'Áudio'], ['video', 'Vídeo'], ['mixed', 'Multimídia']].map(([v, l]) => (
                  <div key={v} className="flex items-center gap-2">
                    ...
                    <Label htmlFor={`fmt-${v}`} className="font-normal cursor-pointer">{l}</Label>
                  </div>
                ))}
              </RadioGroup>
            </div>
